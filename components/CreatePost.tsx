...
export const CreatePost: FC = () => {
  ...
  const mutation = gql`
    mutation createPost($author: ID!, $title: String!, $content: String) {
      createPost(author: $author, title: $title, content: $content) {
        author
        title
        content
      }
  ...
  return (
    <>
      <div className='grid grid-rows-[max-content_max-content] bg-zwav-gray-300 p-[8px] rounded-[8px] gap-y-[6px]'>
        ...
        <button onClick={() => submitPost({ variables: { author: 'unknown', title, content } })} className='w-[80px] rounded-[8px] transition ease-in-out bg-zwav-purple hover:bg-zwav-red duration-300'>
